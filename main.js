...
function calculateIntakeEndDate(intakes, stock, frequency, weekDays) {
  ...
  intakes.forEach(item => {
    ...
    tmpDate.setUTCHours(hours, minutes, 0, 0);
  ...
  return frequency === "weekly"
    ? getWeeklyDate(stock, intakes, weekDays)
    : getDateByDayPeriod(stock, intakes, dayQueue);
}

function getDateByDayPeriod(stock, intakes, n) {
  let sumDays = 0;
  let arePillsExist = stock > 0;
  const pillsDayCount = intakes.reduce(
    (sum, currentItem) => sum + currentItem.pills,
    0
  );
